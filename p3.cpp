  ...
  for(int i = H1 ; i <= H2 ; i++) {
    for(int j = W1 ; j <= W2 ; j++) {
      ...
      Eigen::Vector3d RGB(r, g, b);
        ...
        int getL = 0;
        Luv(getL) = floor(Luv(getL));
        int index = (int) Luv(getL);
        h_i[index] += 1;
    }
  }
    for(int x = 0; x <= 100; x++) {
    ...
    cout << "\n";
     */
    double num_pixels = f_i[100];
    //cout << "num pixels " << num_pixels << "\n";
    for(int x = 0; x <= 100; x++) {
        double A = 0.0;
        if (x - 1 >= 0) {
            A = f_i[x-1];
        }
        double numerator = (A+f_i[x])*100.0;
        double denominator = 2*num_pixels;
        // cout << "numerator ";
        // cout << numerator;
        // cout << " denom ";
        // cout << denominator;
        int res = floor(numerator/denominator);
        lookup_table[x] = res;
    }
  ...
  for(int i = H1 ; i <= H2 ; i++) {
    for(int j = W1 ; j <= W2 ; j++) {
      ...
      Eigen::Vector3d Luv = RGB_to_Luv(RGB);
        ...
        //cout << "before " << Luv(0) << " ";
        int index = (int) Luv(0);
        Luv(0) = (double) lookup_table[index];
        // cout << "after " << Luv(0) << " ";
        // cout << "lookup table " << lookup_table[index] << " ";
        // cout << "index " << index << " ";
